...
Hooks вызываются Claude Code на события жизненного цикла. Если хочется реактивно реагировать на действия агента — auto-format, security scan, push-уведомления, аудит — это правильный механизм.
...
Claude отправляет `Notification`-событие при запросе разрешения, и `Stop` при завершении турна. Hook ловит оба и шлёт push через [ntfy.sh](https://ntfy.sh) — бесплатный сервис, работает без аккаунта, у них есть мобильное приложение.

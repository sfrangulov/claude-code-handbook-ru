...
Для команд и настроек начни с [официальной документации](https://code.claude.com/docs/ru/quickstart). Здесь я помогаю выбрать инструменты, собрать их под свою работу и понять, что оказалось лишним.
...
- **Агент ошибается.** Начни с [правил проекта в CLAUDE.md](#claudemd-шаблоны). В разделе [Evals](#evals) описано, как проверить, помогли ли изменения.
...
- [Терминал и сессии агентов](#терминал-и-сессии-агентов) — runtime под агентами: herdr и tmux
...
|---|---|---:|
| [mattpocock/skills@grill-with-docs](https://skills.sh/mattpocock/skills/grill-with-docs) | «Допрашивай» документацию через find/grep — заменяет догадки точными цитатами. Особенно ценно для библиотек, где знание Claude устарело. | **1.0M** |
| [mattpocock/skills@tdd](https://skills.sh/mattpocock/skills/tdd) | TDD-цикл (red-green-refactor) с дисциплиной — не даёт агенту писать код раньше тестов. От Matt Pocock. | **938K** |
| [anthropics/skills@frontend-design](https://skills.sh/anthropics/skills/frontend-design) | Помогает сделать интерфейс выразительнее. Используй, если результат выглядит шаблонно, например когда Claude снова собрал страницу из одинаковых карточек. Для React и Tailwind. | **906K** |
| [vercel-labs/agent-skills@vercel-react-best-practices](https://skills.sh/vercel-labs/agent-skills/vercel-react-best-practices) | React/Next.js — производительность по гайду Vercel: правильные границы между клиентом и RSC, кэширование, оптимизация размера бандла. Подключай в любом Next.js-проекте. | **730K** |
| [vercel-labs/agent-skills@web-design-guidelines](https://skills.sh/vercel-labs/agent-skills/web-design-guidelines) | Чек-лист соответствия Web Interface Guidelines: a11y, hit-targets, focus rings. Запускай как ревью UI до коммита. | **652K** |
| [mattpocock/skills@codebase-design](https://skills.sh/mattpocock/skills/codebase-design) | Единый словарь для проектирования «глубоких модулей»: много поведения за узким интерфейсом, шов в чистом месте. Берёшь, когда спор идёт о границах, а команда называет одно и то же то компонентом, то сервисом, то API. | **650K** |
| [microsoft/azure-skills@azure-deploy](https://skills.sh/microsoft/azure-skills/azure-deploy) | Деплой в Azure: ARM/Bicep, App Service, Container Apps. Ставь только если работаешь в Azure-стеке — иначе мёртвый груз в контексте. | **591K** |
| [mattpocock/skills@code-review](https://skills.sh/mattpocock/skills/code-review) | Ревью диффа от заданной точки сразу по двум осям: соответствие стандартам репозитория и соответствие исходной задаче. Оси гоняются параллельными субагентами, чтобы не засоряли контекст друг другу. | **588K** |
| [leonxlnx/taste-skill@design-taste-frontend](https://skills.sh/leonxlnx/taste-skill/design-taste-frontend) | Выводит дизайн-направление из брифа и собирает не-шаблонный интерфейс; на редизайне сначала аудитит текущий. Для лендингов и портфолио, когда generic-вид не годится. | **501K** |
| [lllllllama/rigorpilot-skills@minimal-run-and-audit](https://skills.sh/lllllllama/rigorpilot-skills/minimal-run-and-audit) | Воспроизведение ML-репозитория по README с проверяемыми следами: прогоняет документированную команду инференса или smoke-тест и складывает нормализованные артефакты в `repro_outputs/`, отмечая, какие файлы репозитория пришлось править. Узкая ниша — ресёрч-репродукция, а не обычная разработка. | **450K** |
| [mattpocock/skills@git-guardrails-claude-code](https://skills.sh/mattpocock/skills/git-guardrails-claude-code) | Ставит PreToolUse-хук, который блокирует `git push`, `reset --hard`, `clean -f` и `branch -D` до выполнения. Дешёвая страховка, если гоняешь агента в обход подтверждений. | **381K** |
| [obra/superpowers@brainstorming](https://skills.sh/obra/superpowers/brainstorming) | Структурированный брейншторм с гипотезами и матрицей вариантов **до** начала кода. Включай, когда задача расплывчатая («сделай auth»). | **370K** |
| [leonxlnx/taste-skill@high-end-visual-design](https://skills.sh/leonxlnx/taste-skill/high-end-visual-design) | Конкретные шрифты, отступы, тени и анимации, от которых сайт выглядит дорого; убирает дефолты, по которым виден AI-дизайн. Когда нужен premium-look, а не просто «аккуратно». | **368K** |
| [nextlevelbuilder/ui-ux-pro-max-skill@ui-ux-pro-max](https://skills.sh/nextlevelbuilder/ui-ux-pro-max-skill/ui-ux-pro-max) | Дизайн-интеллект для UI/UX сразу под несколько платформ: макеты, компоненты и правила, чтобы интерфейс не выглядел дефолтным. 127k⭐. | **364K** |
| [leonxlnx/taste-skill@redesign-existing-projects](https://skills.sh/leonxlnx/taste-skill/redesign-existing-projects) | Аудит живого интерфейса: находит generic AI-паттерны и подтягивает до premium, не ломая функциональность. Для редизайна существующего проекта, а не старта с нуля. | **364K** |
...
**Совет практика:** ставь `obra/superpowers` целиком сразу — все 14 скиллов образуют один маршрут (TDD, отладка, планирование, брейншторм, код-ревью). В топ-15 из них держится только `brainstorming` — остальные тринадцать в рейтинг не попадают и берут связностью, а не установками; выдирать их по одному смысла мало ([почему](./docs/skill-families-ru.md#obrasuperpowers--методология-которая-владеет-процессом)). Если свой процесс уже есть и рельсы не нужны — бери вместо этого `mattpocock/skills` и вызывай по одному ([чем они отличаются](./docs/skill-families-ru.md#одна-таблица-чем-отличаются)). Дальше добавь скиллы под свой стек (Vercel React, Convex, Firebase, Supabase, Azure). Не ставь всё подряд — каждый скилл съедает 3–5K токенов на bootstrap.
...
- [obra/superpowers](https://github.com/obra/superpowers) — 14 связанных скиллов, образующих методологию: брейншторм → план → TDD → ревью → мерж. Срабатывают автоматически. Теперь в официальном маркетплейсе: `/plugin install superpowers@claude-plugins-official`. [Детальный разбор](./docs/skill-families-ru.md#obrasuperpowers--методология-которая-владеет-процессом).
...
Оркестратор распределяет задачи между агентами. А herdr и tmux управляют терминалами, в которых те работают. Их можно использовать вместе с оркестратором.

Такие инструменты пригодятся в следующих ситуациях:

1. **Закрыл ноутбук или оборвался SSH — работа встала.** Агент живёт в процессе терминала, а тот привязан к клиенту.
2. **Открыто шесть панелей, и непонятно, какая ждёт ответа.** Агент задал вопрос и молча стоит, пока ты смотришь в другую.
3. **Агенту нужно запустить интерактивную программу и увидеть, что она вывела.** Обычный `Bash`-тул этого не умеет: процесс не отпускает управление.

### herdr — runtime, на котором живут агенты

**[herdrdev/herdr](https://github.com/herdrdev/herdr)** — 37.3k⭐, один Rust-бинарь без Electron, Apache-2.0. Не оборачивает и не заменяет агентов, а **владеет их терминалами**: запускает то, что ты и так запускаешь — Claude Code, Codex, Cursor, OpenCode, Grok.

- **Detach без остановки работы.** Терминалы держит фоновый сервер: закрыл клиент или потерял SSH — агенты продолжают. После рестарта машины herdr восстанавливает раскладку и умеет возобновлять сессии поддерживаемых агентов (сами процессы, разумеется, не переживают перезагрузку).
- **Несколько машин в одном окне.** Локальная работа и сохранённые SSH-машины — общий список агентов с независимыми переподключениями.
- **Статус каждой панели.** `working` · `blocked` · `idle`. Застрявшего искать не надо: herdr сам говорит, кто ждёт ответа.
- **Agent-native.** Через CLI и socket API **агенты управляют herdr сами**: спавнят панели, пишут друг другу и ждут, пока другой действительно заблокируется. Есть [готовый скилл](https://herdr.dev/docs/agent-skill/) — это и превращает мультиплексор в оркестратор.
- **Клавиатура и мышь равноправны.** Tmux-style prefix-клавиши *и* клик, драг, сплит.
...
Документация — [herdr.dev/docs](https://herdr.dev/docs/), из неё в первую очередь стоит прочитать [session state](https://herdr.dev/docs/session-state/) (что именно переживает рестарт) и [socket api](https://herdr.dev/docs/socket-api/).

#### Плагины и клиенты

Вокруг herdr выросла экосистема: доступ с телефона, ревью диффов, браузер в панели. Ниже — по одному на сценарий; полный список в [маркетплейсе](https://herdr.dev/plugins/) и в кураторском [yigitkonur/awesome-herdr](https://github.com/yigitkonur/awesome-herdr) (тулы, конфиги, клиенты, скиллы, интеграции).

- [missuo/herdrm](https://github.com/missuo/herdrm) — Нативная macOS-консоль: все агенты и их живые терминалы, в том числе на других машинах. 668⭐.
...
Есть и более нишевое: [herdr-nvim](https://github.com/ChmaraX/herdr-nvim) встраивает Neovim в workspace, [herdr-board](https://github.com/nelsonPires5/herdr-board) делает kanban, где карточка — промпт в видимой панели, [herdr-spreader](https://github.com/yuk1ty/herdr-spreader) поднимает всю раскладку из одного YAML, а [Heeler](https://github.com/ZingerLittleBee/Heeler) — нативная iOS-консоль с настоящим терминалом на libghostty.

### tmux — если новый runtime не нужен

tmux уже стоит у всех, и половина ниши построена поверх него. Начинать стоит с `tmux-cli`: это не оркестратор, а **примитив** — он даёт агенту руки, чтобы запустить интерактивную программу и прочитать её вывод. Именно на таком примитиве держатся скиллы верификации, о которых [Thariq рассказывает](./docs/tips-ru.md#thariq--как-мы-используем-скиллы-внутри-anthropic--17-марта-2026) как о внутренней практике Anthropic (`tmux-cli-driver` в их списке примеров).

- [pchalasani/claude-code-tools → tmux-cli](https://github.com/pchalasani/claude-code-tools) — Даёт агенту управлять интерактивными программами: `launch`, `send`, `capture`, `wait_idle`, `interrupt`, `kill` по панелям. Ровно тот примитив, на котором строятся скиллы верификации. 2.0k⭐.
- [awslabs/cli-agent-orchestrator](https://github.com/awslabs/cli-agent-orchestrator) — Супервайзер делегирует работу агентам-специалистам параллельно или последовательно, каждый — в изолированной tmux-сессии со своей аутентификацией. От AWS Labs. 1.2k⭐.
- [yohey-w/multi-agent-shogun](https://github.com/yohey-w/multi-agent-shogun) — Иерархия сёгун → каро → асигару поверх tmux: параллельные задачи с явными уровнями подчинения. 1.4k⭐.
- [Ataraxy-Labs/opensessions](https://github.com/Ataraxy-Labs/opensessions) — Tmux-сайдбар с маркерами по тредам, живым состоянием сессий и локальным HTTP API. Поддерживает Amp, Codex и OpenCode наравне с Claude Code. 1.2k⭐.
- [Dicklesworthstone/claude_code_agent_farm](https://github.com/Dicklesworthstone/claude_code_agent_farm) — Ферма из 20+ параллельных агентов: координация через локи, прогоны по багам и best practices, мониторинг в tmux. 916⭐.
- [alexei-led/ccgram](https://github.com/alexei-led/ccgram) — Мост Telegram ↔ tmux/herdr: читать вывод, отвечать на запросы агента и вести параллельные сессии с телефона. 263⭐.

**Критерий отбора:** пуш за последние три месяца и явная поддержка Claude Code (данные — на 2026-09-10). В нише особенно много мёртвого: самый популярный в выдаче tmux-оркестратор (1.8k⭐) не получал коммитов больше года, поэтому его здесь нет. Проверяй дату последнего пуша до установки, а не после.
...
> **Правило практика:** одного оркестратора достаточно. Не комбинируй — все три семьи конфликтуют за worktrees, лимиты API и runtime. Выбери по таблице ниже. Оговорка: [herdr и tmux](#терминал-и-сессии-агентов) из раздела выше — не четвёртая семья, а слой под ними, и с любым из этих оркестраторов он уживается.

### Сравнение под соло-разработчика

Звёзды и дата последнего пуша — на 2026-09-10, GitHub API. Сложность — субъективная оценка времени до первого полезного запуска. Смотри на пуш раньше, чем на звёзды: половина ниши живёт один сезон. ⚠️ — больше четырёх месяцев без коммитов.

| Тулза | ⭐ | Пуш | «Забыл и ушёл» | Сложность | Под кого |
|---|---:|---|---|---|---|
| [gastownhall/gastown](https://github.com/gastownhall/gastown) | 17.9k | 2026-09-10 | ✅ настоящий | Высокая | Overkill для одиночки, под сложные multi-agent сценарии |
| [sipyourdrink-ltd/bernstein](https://github.com/sipyourdrink-ltd/bernstein) | 1.1k | 2026-09-10 | ✅ настоящий | Средняя | Соло-разработчик с требованием audit-grade лога |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | 71.8k | 2026-09-10 | ✅ swarm | Высокая | Команды и enterprise, не для одиночки |
| [BloopAI/vibe-kanban](https://github.com/BloopAI/vibe-kanban) | 28.0k | 2026-04-24 ⚠️ | ❌ полу-ручное | Низкая | Простая параллельность через kanban-доску |
| [smtg-ai/claude-squad](https://github.com/smtg-ai/claude-squad) | 8.4k | 2026-08-20 | ❌ ручное | Низкая | Несколько Claude-сессий в TUI |
| [stravu/crystal (Nimbalyst)](https://github.com/stravu/crystal) | 3.1k | 2026-02-26 ⚠️ | ❌ полу-ручное | Низкая | Desktop GUI для сравнения подходов |
| [manaflow-ai/cmux](https://github.com/manaflow-ai/cmux) | 26.9k | 2026-09-10 | ❌ ручное | Низкая | macOS-юзеру с табами и push-нотификациями |
| [generalaction/emdash](https://github.com/generalaction/emdash) | 5.7k | 2026-09-09 | ❌ полу-ручное | Низкая | Open-source альтернатива vibe-kanban |
...
| [humanlayer/humanlayer](https://github.com/humanlayer/humanlayer) | 11.5k | 2026-06-19 | ⚠️ approval-gated | Средняя | Сложные кодовые базы с обязательными человеческими чекпоинтами |
...
Готовые процессы разработки для Claude Code. Они ведут агента от изучения задачи и плана до кода, проверки и выпуска. Такой процесс можно установить как плагин или набор скиллов.
...
>
> 🇷🇺 **Переводы тематических батчей советов** от Boris Cherny (создатель Claude Code) и Thariq (Anthropic): **[docs/tips-ru.md →](./docs/tips-ru.md)** — 8 батчей за январь–апрель 2026, разбор трёх фаз воркшопа How We Claude Code (Code with Claude 2026) и статья Anthropic о снижении стоимости без потери качества (сентябрь 2026).
...
>
> Ограничивать себя всё равно стоит, но из-за доверия и промахов, а не из-за токенов: каждый сервер — чужой код с доступом к твоему окружению, и чем больше похожих тулов, тем чаще модель берёт не тот. Какие из подключённых реально вызываются, покажет мой [`npx mcp-graveyard`](https://github.com/sfrangulov/skill-graveyard/tree/main/packages/mcp-graveyard) — аудит по локальным логам сессий, без сети и телеметрии.
...
- **Локальный RAG** — [sfrangulov/minirag-mcp](https://github.com/sfrangulov/minirag-mcp). Гибридный поиск (семантика + BM25) по своим документам: база знаний проекта, 12 входных форматов через markitdown, ничего не уходит с машины. Мой проект.
...
### Не только считать, но и снижать

Инструменты выше отвечают на вопрос «сколько потратил». Вопрос «почему столько и как меньше» закрывает официальный скилл [`claude-api`](https://github.com/anthropics/skills/tree/main/skills/claude-api) тремя командами:

- **`/claude-api prompt-audit`** — вычищает из промптов, скиллов и `CLAUDE.md` анти-паттерны, оставшиеся от прошлых поколений моделей: ритуалы «проверь дважды», scratchpad-скаффолды, противоречивые правила, снятые thinking-настройки. На замере Anthropic при миграции Opus 4.8 → Opus 5 это дало −14.6% стоимости **и** +5.3% точности.
- **`/claude-api cost-optimize`** — аудит расхода приложения на Claude API: профилирует, куда уходят токены, и применяет экономию (кэш, батчинг, ограничение вывода). На публичных бенчмарках −52…−73% при качестве в пределах доверительного интервала.
- **`/claude-api hillclimb`** — поиск по сетке моделей и уровней effort на твоём евале, с train/test-разбиением и чтением провалившихся примеров.

Разбор всех трёх рычагов — prompt cache, анти-паттерны, калибровка effort — с цифрами: **[docs/tips-ru.md → партия от 8 сентября 2026](./docs/tips-ru.md#anthropic--снижение-стоимости-без-потери-качества--8-сентября-2026)**.
...
- [jackwener/OpenCLI](https://github.com/jackwener/OpenCLI) — Агент работает с сайтами через твой залогиненный Chrome: расширение-мост и команды `opencli browser` — открыть, кликнуть, заполнить форму, вытащить данные, посмотреть сеть. Для частых задач — детерминированные адаптеры вида `opencli hackernews top`, свои пишутся по скиллу `opencli-adapter-author`, скиллы ставятся через `npx skills add jackwener/opencli`. Сайт — [opencli.info](https://opencli.info/). 29k⭐.
- [Panniantong/Agent-Reach](https://github.com/Panniantong/Agent-Reach) — Чтение и поиск по вебу одним инструментом: страницы, субтитры YouTube, RSS, GitHub, X, Reddit, LinkedIn. Для каждой площадки держит основной и запасной способ доступа, для сайтов с логином берёт OpenCLI; `agent-reach doctor` показывает, что работает. Когда брать: агенту надо читать и искать, а не кликать. Ставится агентом по ссылке на `install.md` — прочитай файл до того, как отдать ссылку. 82k⭐.

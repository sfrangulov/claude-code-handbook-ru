...
Если нашёл полезный скилл, статью или инструмент для Claude Code, добавь его через PR. В описании напиши, что он делает и когда пригодится.
...
2. Открой подходящий файл и добавь запись:
...
3. Запусти `node scripts/build-readme.mjs` — он перегенерирует `README.md`, соответствующий `catalog/*.md` и `llms.txt` + `llms-full.txt` (последние два — для LLM/агентов, по спецификации llmstxt.org) из template + data.
4. Закоммить `data/<секция>.json` и все обновлённые сгенерённые файлы, которые изменились в diff'е.
5. Сохраняй алфавитный порядок внутри подсекции, где это применимо.
6. Открой PR с описанием: что добавил и почему это полезно.
...
Строгость зависит от того, куда попала запись: в курируемой подборке (`data/*.json`, `README.template.md`) переименование или архив — ошибка, в сыром каталоге (`data/catalog/*.json`) те же находки идут в отчёт и CI не ломают. Если проект заброшен, но оставлен намеренно — впиши его в [`.github/repo-check-ignore.json`](./.github/repo-check-ignore.json) с причиной, и предупреждение уйдёт.

Топ-15 скиллов обновляется автоматически: cron-workflow [`refresh-skills-top.yml`](./.github/workflows/refresh-skills-top.yml) раз в неделю опрашивает skills.sh. Обновление одних лишь чисел уходит в `main` напрямую ботом — это механическая сверка с внешним источником. PR открывается только когда в топ вошёл **новый** скилл: его описание приезжает заглушкой, и написать его должен человек. Вручную:
...
Если у тебя есть собственные скиллы или команды, которые хочется опубликовать как часть этого списка — клади их в `examples/` через PR. Структура:
...
Если не уверен, открой Issue с тегом `[предложение]` до PR.
